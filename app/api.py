...
def create_api_router(repository: TicketRepository) -> APIRouter:
    ...
    def list_tickets(
        ...
        return tickets.list(filters if any(filters.model_dump().values()) else None)
    ...
    def get_ticket(ticket_id: TicketId, tickets: TicketRepository = Depends(get_repository)) -> Ticket:
        try:
    ...
    def delete_ticket(ticket_id: TicketId, tickets: TicketRepository = Depends(get_repository)) -> Response:
        try:
